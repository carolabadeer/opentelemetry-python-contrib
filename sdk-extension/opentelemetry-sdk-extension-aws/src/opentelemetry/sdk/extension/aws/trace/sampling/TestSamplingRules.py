import unittest
# from opentelemetry.sdk.extension.aws.trace.sampling import GetSamplingRulesRequest
...
from AWSXRayRemoteSampler import AWSXRayRemoteSampler


class TestGetSamplingRulesResponse(unittest.TestCase):
    def setUp(self):
        self.rules_response = GetSamplingRulesResponse.create(
            "test token", ["test sampling rule"])

    def test_token(self):
        self.assertEqual(self.rules_response.get_next_token(),
                         "test token")

    def test_sampling_rules(self):
        self.assertEqual(
            self.rules_response.get_sampling_rules(), ["test sampling rule"])


class TestSamplingRuleRecord(unittest.TestCase):
    def setUp(self):
        self.rule_record = SamplingRuleRecord.create(
            10, 11, ["sampling rule 1", "sampling rule 2"])
    ...
    def test_sampling_rule(self):
        self.assertEqual(self.rule_record.get_sampling_rule(), [
                         "sampling rule 1", "sampling rule 2"])
...
class TestSamplingRule(unittest.TestCase):
    ...
    def test_attributes(self):
        self.assertEqual(self.sampling_rule.get_attributes(), {
                         "attr1": "test attr1", "attr2": "test attr2"})
    ...
    def test_resourceArn(self):
        self.assertEqual(self.sampling_rule.get_resource_arn(),
                         "resource arn test")
    ...
    def test_serviceName(self):
        self.assertEqual(self.sampling_rule.get_service_name(),
                         "service name test")

    def test_serviceType(self):
        self.assertEqual(self.sampling_rule.get_service_type(),
                         "service type test")
...
class TestRulePoller(unittest.TestCase):
    def setUp(self):
        self.remote_sampler = AWSXRayRemoteSampler()

    def test_rulePollerInitiation(self):
        self.remote_sampler.get_and_update_sampling_rules()
